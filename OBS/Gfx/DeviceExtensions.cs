...
namespace iSee.External.Gfx
{
    ...
    public static class DeviceExtensions
    {
        ...
        {
            var verticesScreen = verticesWorld.Select(v => graphics.CSGOData.Player.MatrixViewProjectionViewport.Transform(v)).ToArray();
        ...
        public static void DrawText(this Graphics graphics, string text, Color color, params Vector3[] verticesWorld)
        {
            var verticesScreen = verticesWorld.Select(v => graphics.CSGOData.Player.MatrixViewProjectionViewport.Transform(v)).ToArray();
            graphics.FontVerdana8.DrawText(default, text, new Point((int)verticesScreen[0].X-10, (int)verticesScreen[0].Y-55), color);
        }
    }
}
