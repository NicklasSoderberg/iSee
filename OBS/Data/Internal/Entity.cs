...
namespace iSee.External.Data.Internal
{
        ...
        EntityBase
    {
        ...
        {
            return CSGOProcess.ModuleClient.Read<IntPtr>(Offsets.dwEntityList + Index * 0x10 /* size */);
        }
        ...
        public override bool Update(CSGOProcess CSGOProcess)
        {
            ...
            Dormant = CSGOProcess.Process.Read<bool>(AddressBase + Offsets.m_bDormant);
            if (!IsAlive())
            {
                return true;
            }

            UpdateStudioHdr(CSGOProcess);
            UpdateStudioHitBoxes(CSGOProcess);
            UpdateStudioBones(CSGOProcess);
            UpdateBonesMatricesAndPos(CSGOProcess);
            UpdateSkeleton();
        ...
        /// </summary>
        private void UpdateStudioHdr(CSGOProcess CSGOProcess)
        {
            var addressToAddressStudioHdr = CSGOProcess.Process.Read<IntPtr>(AddressBase + Offsets.m_pStudioHdr);
            AddressStudioHdr = CSGOProcess.Process.Read<IntPtr>(addressToAddressStudioHdr); // deref
            StudioHdr = CSGOProcess.Process.Read<studiohdr_t>(AddressStudioHdr);
        }

        /// <summary>
        /// Update <see cref="StudioHitBoxSet"/> and <see cref="StudioHitBoxes"/>.
        /// </summary>
        private void UpdateStudioHitBoxes(CSGOProcess CSGOProcess)
        {
            var addressHitBoxSet = AddressStudioHdr + StudioHdr.hitboxsetindex;
            StudioHitBoxSet = CSGOProcess.Process.Read<mstudiohitboxset_t>(addressHitBoxSet);

            // read
            for (var i = 0; i < StudioHitBoxSet.numhitboxes; i++)
            {
                StudioHitBoxes[i] = CSGOProcess.Process.Read<mstudiobbox_t>(addressHitBoxSet + StudioHitBoxSet.hitboxindex + i * Marshal.SizeOf<mstudiobbox_t>());
            }
        }
        ...
        /// </summary>
        private void UpdateStudioBones(CSGOProcess CSGOProcess)
        {
            for (var i = 0; i < StudioHdr.numbones; i++)
            {
                StudioBones[i] = CSGOProcess.Process.Read<mstudiobone_t>(AddressStudioHdr + StudioHdr.boneindex + i * Marshal.SizeOf<mstudiobone_t>());
            }
        }
        ...
        /// </summary>
        private void UpdateBonesMatricesAndPos(CSGOProcess CSGOProcess)
        {
            var addressBoneMatrix = CSGOProcess.Process.Read<IntPtr>(AddressBase + Offsets.m_dwBoneMatrix);
            for (var boneId = 0; boneId < BonesPos.Length; boneId++)
            {
                var matrix = CSGOProcess.Process.Read<matrix3x4_t>(addressBoneMatrix + boneId * Marshal.SizeOf<matrix3x4_t>());
